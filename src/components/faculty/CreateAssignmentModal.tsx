      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
          <motion.div
            ...
            className="relative w-full max-w-2xl max-h-[92vh] flex flex-col rounded-3xl border border-cyan-500/40 bg-slate-950 p-6 shadow-[0_0_50px_rgba(6,182,212,0.25)] overflow-hidden"
          >
            {/* Header */}
            <div className="flex items-start justify-between pb-4 border-b border-white/10">
              <div>
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="text-lg font-bold text-white">Create & Assign Coursework</h3>
                </div>
            ...
            <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto space-y-4 my-3 pr-1">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  <label className="text-xs font-semibold text-zinc-300 mb-1 block">Assigned Subject</label>
                  <select
                    value={subjectId}
                ...
                <div>
                  <label className="text-xs font-semibold text-zinc-300 mb-1 block">Coursework Category</label>
                  <select
                    value={kind}
              ...
              <div>
                <label className="text-xs font-semibold text-zinc-300 mb-1 block">Assignment Title</label>
                <Input
              ...
              <div>
                <label className="text-xs font-semibold text-zinc-300 mb-1 block">Brief Summary / Instructions</label>
                <Textarea
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <div>
                  <label className="text-xs font-semibold text-zinc-300 mb-1 flex items-center gap-1">
                    <Calendar size={12} className="text-cyan-400" /> Deadline Date
                  </label>
                  <input
                    type="date"
                    value={dueDate}
                ...
                <div>
                  <label className="text-xs font-semibold text-zinc-300 mb-1 flex items-center gap-1">
                    <Clock size={12} className="text-blue-400" /> Due Time
                  </label>
                  <input
                    type="time"
                    value={dueTime}
                ...
                <div>
                  <label className="text-xs font-semibold text-zinc-300 mb-1 flex items-center gap-1">
                    <AlertTriangle size={12} className="text-amber-400" /> Urgency Level ({urgency}/5)
                  </label>
                  <input
                    type="range"
                    min={1}
                    ...
                    className="w-full h-2 mt-3 cursor-pointer appearance-none bg-slate-800 rounded-lg accent-cyan-400"
                  />
                </div>
              </div>
              ...
              <div>
                <label className="text-xs font-semibold text-zinc-300 mb-1 flex items-center gap-1">
                  <FileText size={12} className="text-cyan-400" /> Official Question Paper / Rubric PDF URL (Optional)
                </label>
                <div className="flex gap-2">
                  <Input
                    ...
                    className="bg-black/40 text-xs flex-1"
                  />
                  <button
                    type="button"
                    onClick={() => setQuestionPdfUrl('https://campusflow.edu/materials/cs301_advanced_lab_spec.pdf')}
                    className="px-2.5 py-1 text-xs rounded-xl bg-cyan-500/20 text-cyan-300 border border-cyan-500/30 hover:bg-cyan-500/30 transition-all shrink-0 cursor-pointer"
                  >
                    Sample PDF
                  </button>
                </div>
              ...
              <div className="rounded-2xl border border-white/10 bg-black/30 p-3.5 space-y-3">
                <div className="flex items-center justify-between">
                  <label className="text-xs font-bold uppercase tracking-wider text-cyan-300 flex items-center gap-1">
                    <FileText size={12} /> Specific Questions & Sub-Tasks ({questions.length})
                  </label>
                  <span className="text-[10px] text-zinc-400 font-mono">Visible in student problem brief</span>
                </div>
                ...
                <div className="space-y-2">
                  {questions.map((q, idx) => (
                    <div
                      key={idx}
                      className="flex items-start justify-between gap-2 p-2.5 rounded-xl bg-slate-900/80 border border-white/5 text-xs text-zinc-200"
                    >
                ...
                <div className="space-y-2 pt-1 border-t border-white/5">
                  <div className="flex gap-2">
                    <Input
                      ...
                      className="bg-black/50 text-xs flex-1"
                    />
                    <Button
                      type="button"
                      size="sm"
                  ...
                  <div className="flex flex-wrap items-center gap-3 text-[11px] text-zinc-400 bg-black/40 p-2 rounded-xl border border-white/5">
                    <span className="font-semibold text-cyan-300">OBE Tags:</span>
                    <div className="flex items-center gap-1.5">
                      <span>Course Outcome:</span>
                      <select
                        value={newQuestionCO}
                        onChange={(e) => setNewQuestionCO(e.target.value)}
                        className="bg-slate-900 text-cyan-400 font-mono text-[11px] rounded px-1.5 py-0.5 border border-white/10"
                      >
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      <select
                        ...
                        className="bg-slate-900 text-purple-400 font-medium text-[11px] rounded px-1.5 py-0.5 border border-white/10"
                      >
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      <input
                        ...
                        className="w-14 bg-slate-900 text-emerald-400 font-mono text-[11px] rounded px-1.5 py-0.5 border border-white/10 text-center"
                      />
                    </div>
                  </div>
                </div>
              ...
              <div className="flex justify-end gap-2 pt-2 border-t border-white/10">
                ...
                <Button type="submit" size="sm" className="bg-cyan-500 text-slate-950 hover:bg-cyan-400 font-bold">
                  Publish to Students
                </Button>
              </div>
            </form>
