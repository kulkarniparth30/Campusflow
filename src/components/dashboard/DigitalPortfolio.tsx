  ...
  return (
    <Card className="h-full flex flex-col relative">
      ...
      <AnimatePresence>
        {open && (
          <motion.form
            ...
            className="mb-4 space-y-3 rounded-2xl border border-cyan-500/30 bg-slate-950/80 p-4 overflow-hidden origin-top"
          ...
          >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
              <select
                ...
                className="h-10 w-full rounded-xl border border-white/10 bg-slate-900 px-3 text-xs text-zinc-200 outline-none focus:border-cyan-400"
              >
              ...
              <div className="flex items-center text-xs text-zinc-400 px-1">
            ...
            <div className="rounded-xl border border-white/10 bg-black/30 p-3 space-y-2">
              <label className="text-xs font-semibold text-zinc-300 flex items-center gap-1.5">
                <Camera size={13} className="text-cyan-400" /> Event Attendance / Participation Photo Proof
              </label>
              <Input
                placeholder="Image URL (e.g. https://... or select below)"
                value={imageProofUrl}
                onChange={(e) => setImageProofUrl(e.target.value)}
                className="bg-black/50 text-xs h-8"
              />
              <div className="flex items-center gap-1.5 flex-wrap pt-1">
                <span className="text-[10px] text-zinc-500">Quick Attach Samples:</span>
                {SAMPLE_EVENT_PHOTOS.map((sp) => (
                  <button
                    ...
                    className="text-[10px] px-2 py-0.5 rounded-lg bg-cyan-500/10 hover:bg-cyan-500/20 text-cyan-300 border border-cyan-500/20 transition-all cursor-pointer"
                  >
                    + {sp.label}
                  </button>
                ))}
              ...
              {imageProofUrl && (
                <div className="mt-2 flex items-center gap-2">
                  <img
                    src={imageProofUrl}
                    alt="Proof preview"
                    className="w-16 h-12 rounded-lg object-cover border border-cyan-500/40"
                  />
            ...
            <div className="rounded-xl border border-white/10 bg-black/30 p-3 space-y-2">
              <label className="text-xs font-semibold text-zinc-300 flex items-center gap-1.5">
                <FileText size={13} className="text-blue-400" /> Certificate Document / Registration Proof
              </label>
              <div className="flex gap-2">
                <Input
                  ...
                  className="bg-black/50 text-xs h-8 flex-1"
                />
                <button
                  type="button"
                  onClick={() => {
                  ...
                  className="text-xs px-2.5 py-1 rounded-xl bg-blue-500/20 text-blue-300 border border-blue-500/30 hover:bg-blue-500/30 transition-all shrink-0 cursor-pointer"
                >
                  Attach PDF
                </button>
              </div>
            ...
            <Textarea
              ...
              className="min-h-16 text-xs bg-black/40"
            />
            ...
            <div className="flex justify-end gap-2 pt-1">
              ...
              <Button size="sm" type="submit" className="bg-cyan-500 text-slate-950 hover:bg-cyan-400 font-bold">
                Submit to Portfolio
              </Button>
            </div>
          </motion.form>
      ...
      <div className="space-y-3 flex-1 overflow-y-auto pr-1">
        {items.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-zinc-500 space-y-2 py-12 opacity-60">
          ...
          items.map((a, i) => {
            ...
            return (
              <motion.div
                ...
                className="group flex flex-col sm:flex-row items-start gap-4 rounded-2xl border border-white/10 bg-slate-900/40 p-4 hover:border-cyan-500/40 hover:bg-slate-900/70 hover:shadow-[0_0_25px_rgba(6,182,212,0.15)] transition-all cursor-pointer"
              >
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center justify-between gap-2">
                    ...
                    {a.verified ? (
                      <Badge tone="emerald" className="shrink-0 flex items-center gap-1 shadow-[0_0_10px_rgba(52,211,153,0.3)]">
                        <ShieldCheck size={11} /> 
                        <span>Verified</span>
                  ...
                  {a.description && (
                    <p className="mt-1 text-xs text-zinc-300/80 line-clamp-2">{a.description}</p>
                  )}

                  <div className="mt-2.5 flex items-center gap-3 text-[11px] text-zinc-400 flex-wrap">
                    <span className="flex items-center gap-1 rounded bg-black/40 px-2 py-0.5 text-zinc-300 font-medium">
                    ...
                    {a.image_proof_url && (
                      <span className="inline-flex items-center gap-1 text-cyan-300 text-[10px] font-semibold bg-cyan-500/10 px-2 py-0.5 rounded-md border border-cyan-500/20">
                        <ImageIcon size={10} /> Photo Proof
                      </span>
                    )}

                    {a.certificate_url && (
                      <span className="inline-flex items-center gap-1 text-blue-300 text-[10px] font-semibold bg-blue-500/10 px-2 py-0.5 rounded-md border border-blue-500/20">
        ...
        {inspectingItem && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
            <motion.div
              ...
              className="relative w-full max-w-2xl max-h-[90vh] flex flex-col rounded-3xl border border-cyan-500/40 bg-slate-950 p-6 shadow-[0_0_50px_rgba(6,182,212,0.25)] overflow-hidden"
            >
              <div className="flex items-start justify-between pb-4 border-b border-white/10">
                <div>
                  ...
                  <p className="text-xs text-zinc-400 mt-1">
                    Issued by <strong className="text-zinc-200">{inspectingItem.issuer}</strong> · Awarded on {inspectingItem.awarded_on || 'Recent'}
                  </p>
                </div>
                <button
              ...
              <div className="flex-1 overflow-y-auto space-y-4 my-3 pr-1">
                ...
                {inspectingItem.description && (
                  <div className="p-3.5 rounded-2xl bg-black/40 border border-white/10 text-xs text-zinc-300 leading-relaxed">
                    <p className="font-semibold text-white mb-1">Credential Summary & Impact:</p>
                    {inspectingItem.description}
                  </div>
                )}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div className="p-3.5 rounded-2xl border border-white/10 bg-black/30 space-y-2">
                    <span className="text-xs font-bold text-zinc-300 uppercase tracking-wider">Faculty Verification</span>
                    <div className="flex items-center gap-2">
                      {inspectingItem.verified ? (
                        <>
                          <CheckCircle2 size={16} className="text-emerald-400 shrink-0" />
                          <span className="text-xs text-emerald-300 font-semibold">Verified & Endorsed by Academic Council</span>
                        </>
                      ) : (
                        <>
                          <Clock size={16} className="text-amber-400 shrink-0" />
                          <span className="text-xs text-amber-300 font-semibold">Pending Faculty Review</span>
                        </>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="p-3.5 rounded-2xl border border-white/10 bg-black/30 space-y-2">
                    <span className="text-xs font-bold text-zinc-300 uppercase tracking-wider">Document Certificate</span>
                    {inspectingItem.certificate_url ? (
                      ...
                      <p className="text-xs text-zinc-500 italic">Self-attested without digital PDF.</p>
                    )}
                  </div>
                </div>
              </div>
